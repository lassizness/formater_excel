...
void __fastcall TForm4::Button3Click(TObject *Sender) {
		...
		for (int j = 1; j <= StringGrid->ColCount; j++) {
			...
			if (dynamic_cast<TCheckBox*>(Panel2->FindComponent("check" +
				...
				Cll = Sht.OlePropertyGet("Cells").OlePropertyGet("Item", i,
					CountCol++);
				Cll.OlePropertySet("RowHeight", 20); //Высота ячейки
				Cll.OlePropertySet("ColumnWidth", 15);// Ширина ячейки
	...
	App.OlePropertyGet("Workbooks").OlePropertyGet("Item", 1).OleProcedure
		...
		ExtractFileName(od->FileName))); //Сохраняем файл EXEL с префексом + исходное имя.
	App.OleProcedure("Quit"); //Закрываем EXEL по окончанию.
